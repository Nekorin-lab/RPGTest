        ...
        public bool FrowIf(string eventId, EventDataModel.EventCommand command) {
            ...
            if (command.parameters[19] == "1")
            {
                ...
                switch (command.parameters[21])
                {
                    ...
                    // スキルを覚えている
                    case "3":
                        for (var i = 0; i < actorData.skills.Count; i++)
                            if (actorData.skills[i] == command.parameters[24])
                            {
                                if (flowType != 1)
                                    return true;
                                isFound = true;
                    ...
                    // ステートが付与されている
                    case "6":
                        for (var i = 0; i < actorData.states.Count; i++)
                            if (actorData.states[i].id == command.parameters[27])
                            {
                                if (flowType != 1)
                                    return true;
                                isFound = true;
