...
namespace RPGMaker.Codebase.Runtime.Battle.Objects
{
    ...
    public class GameAction
    {
        ...
        public void SetAttack() {
            SetSkill(GameBattlerBase.AttackSkillId, TypeEnum.Attack);
        }
        ...
        /// <summary>
        /// ダメージの[タイプ]が[TP回復]か（使用者への影響）
        /// </summary>
        /// <returns></returns>
        public bool IsTpRecoverMyself() {
            return Item.RecoverTpMyself == 1;
        }
        ...
        public bool TestApply(GameBattler target) {
            return IsForDeadFriend() == target.IsDead() &&
                   (
        ...
        public void ApplyMyself(GameBattler target) {
            ...
            if (IsTpRecoverMyself() && GameStateHandler.IsBattle())
            {
                if (Item.IsSkill())
                {
                    var skillMasterData = DataManager.Self().GetSkillCustomDataModel(Item.ItemId);
                    ItemEffectGainTp(target, skillMasterData.userEffect.heal.tp);
                }
                else
                {
                    var itemMasterData = DataManager.Self().GetItemDataModel(Item.ItemId);
                    ItemEffectGainTpItem(target, itemMasterData.userEffect.heal.tp);
                }
            }
        ...
        public void ItemEffectLearnSkill(GameBattler target, TraitCommonDataModel effect) {
            ...
            {
                var actor = (GameActor) target;
                actor.LearnSkill(effect.traitsId);
                MakeSuccess(target);
            }
        }
        ...
        /// </summary>
        /// <param name="target"></param>
        public void ApplyItemUserEffect(GameBattler target) {
        ...
        private float CalcElementRate(GameBattler target) {
            ...
            for (int i = 0; i < elements.Count; i++)
            {
                ...
                rate = rate * target.ElementRate(Item.DamageElementId);
