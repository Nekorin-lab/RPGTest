...
namespace RPGMaker.Codebase.Runtime.Battle.Objects
{
    ...
    public class GameItem {
        ...
        /// 使用者への影響 TP回復するかどうか
        /// </summary>
        public int RecoverTpMyself
        {
            get
            {
                if (IsSkill()) return SkillData.userEffect.heal.tp.enabled;
                else if (IsItem()) return ItemData.userEffect.heal.tp.enabled;
                return 0;
            }
        }
        ...
        /// <summary>
        /// 対象者への影響 攻撃タイプ
        /// Uniteで設定した攻撃タイプを、MVの設定値に変換の上で返却する
        /// </summary>
        public int AttackType
        {
            get
            {
                if (IsSkill()) return ConvertUniteData.SetDamageType(SkillData.targetEffect.damage.attackType);
                return -1;
            }
        }
