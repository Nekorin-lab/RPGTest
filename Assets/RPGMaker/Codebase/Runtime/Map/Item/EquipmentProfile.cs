...
namespace RPGMaker.Codebase.Runtime.Map.Item
{
    public class EquipmentProfile : MonoBehaviour
    {
        ...
        //→のステータス
        public void EqValuesDisplay(List<int> parameters, string typeId) {
            // 装備中のアイテムを取得
            var equipParam = new List<int> {0, 0, 0, 0, 0, 0, 0, 0};
            for (var i = 0; i < _runtimeActorDataModel.equips.Count; i++)
                if (_runtimeActorDataModel.equips[i].equipType == typeId)
                {
                    if (_runtimeActorDataModel.equips[i].itemId == "") break;

                    var weapons = _databaseManagementService.LoadWeapon();
                    WeaponDataModel weapon = null;
                    for (int i2 = 0; i2 < weapons.Count; i2++)
                        if (weapons[i2].basic.id == _runtimeActorDataModel.equips[i].itemId)
                        {
                            weapon = weapons[i2];
                            break;
                        }
                    ...
                    if (weapon == null)
                    {
                        ...
                        for (int i2 = 0; i2 < armors.Count; i2++)
                            if (armors[i2].basic.id == _runtimeActorDataModel.equips[i].itemId)
                            {
                                armor = armors[i2];
                                break;
                            }
