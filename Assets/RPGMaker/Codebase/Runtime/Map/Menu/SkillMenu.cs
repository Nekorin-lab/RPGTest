    ...
    public class SkillMenu : WindowBase
    {
        ...
        public void Init(MenuBase @base, string actorId) {
            ...
            for (int i = 1; i < skillCommands.Count; i++)
            {
                ...
                obj.transform.parent = _commandOrigin.transform.parent;
        ...
        public void CreateItem(int type) {
            ...
            for (var i = 0; i < _runtimeActorDataModel.skills.Count; i++)
            {
                var flg = false;
                for (var j = 0; j < classIds.Count; j++)
                    if (classIds[j] == _runtimeActorDataModel.skills[i])
                    {
                        flg = true;
                        break;
                    }
                ...
                if (!flg)
                {
                    var skill = DataManager.Self().GetSkillCustomDataModel(_runtimeActorDataModel.skills[i]);
                    if (skill != null)
                    {
            ...
            for (var i = 0; i < skillData.Count; i++)
            {
                ...
                obj.transform.parent = _originObj.transform.parent;
