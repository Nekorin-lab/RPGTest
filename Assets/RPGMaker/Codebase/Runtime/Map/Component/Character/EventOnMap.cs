    ...
    public class EventOnMap : CharacterOnMap
    {
        ...
        */
        public override void UpdateTimeHandler() {
            ...
            //現在、イベントが一時消去状態かつ、無効状態であれば更新処理を停止する
            /*
            if (!isValid && MapDataModelEvent.temporaryErase == 1)
            {
                TimeHandler.Instance?.RemoveTimeAction(UpdateTimeHandler);
            }
            */
            ...
            _moveEvent.SetNpcMove(MapDataModelEvent.pages[page], MoveEnd, () =>
            {
                ...
                if (isValid && MapDataModelEvent.pages[page].eventTrigger == (int) EventTriggerEnum.ContactFromTheEvent)
                {
                    ...
                    if (characterOnMap.IsSameCheck(x_now, y_now) && MapDataModelEvent.pages[page].priority != 1)
                        _eventCommandChainLauncher.LaunchCommandChain(MapDataModelEvent, EventDataModel);
                    else if (characterOnMap.IsAroundCheck(x_now, y_now) && MapDataModelEvent.pages[page].priority == 1)
                        _eventCommandChainLauncher.LaunchCommandChain(MapDataModelEvent, EventDataModel);
                }
        ...
        public bool CheckValid(bool isForce = false) {
            ...
            SetMovementParameter(isForce);
            return isValid;
        }

        private void SetMovementParameter(bool isForce = false) {
            try
            {
                //現在、自分自身が破棄済みであれば処理しない
                if (gameObject == null || transform == null) return;
                //現在、自分自身のイベントが実行中であれば、更新処理を行わない
                if (!isForce && _eventCommandChainLauncher.IsRunning()) return;
                //現在、イベントが一時消去状態かつ、無効状態であれば、更新処理を行わない
                if (!isValid && MapDataModelEvent.temporaryErase == 1) return;
            }
            catch (Exception)
            {
            ...
            //if (saveDataWork == null || sw == null || vari == null) return;
