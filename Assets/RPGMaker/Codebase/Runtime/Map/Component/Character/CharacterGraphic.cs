...
namespace RPGMaker.Codebase.Runtime.Map.Component.Character
{
    public class CharacterGraphic : MonoBehaviour
    {
        ...
        protected SkeletonAnimation _skelAnim;
        ...
        public virtual void Init(string assetId, bool isFirstInit = true) {
            ...
            _skelAnim = gameObject.GetOrAddComponent<SkeletonAnimation>();

            string skelpath = "Assets/Spine/Sonia/Branche_Princess_SkeletonData.asset";
            SkeletonDataAsset skelAsset = AssetDatabase.LoadAssetAtPath<SkeletonDataAsset>(skelpath);
 
            _skelAnim.skeletonDataAsset = skelAsset;
            _skelAnim.initialSkinName = "Branche_Princess_2";

            gameObject.GetOrAddComponent<MeshRenderer>().sortingLayerID = UnityUtil.SortingLayerManager.GetId(UnityUtil.SortingLayerManager.SortingLayerIndex.Runtime_MapCharacterPriorityUnder);
            _skelAnim.Awake();
            // サイズ、位置設定
            transform.GetComponent<RectTransform>().pivot = new Vector2(0, 0);
            transform.GetComponent<RectTransform>().sizeDelta = new Vector2(1, 1);
        ...
        public virtual void StepAnimation() {
            ...
            if (animationFrameCount == 0 || _frameCount % animationFrameCount == 0)
            {
