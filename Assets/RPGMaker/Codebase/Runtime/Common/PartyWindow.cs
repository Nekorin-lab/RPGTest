...
namespace RPGMaker.Codebase.Runtime.Common
{
    public class PartyWindow : WindowBase
    {
        ...
        public void Init(PartyType type, string useActorId, string useId, GameItem gameItem, Action callback) {
            ...
            for (var i = 0; i < 4; i++)
            {
                if (i < _runtimeSaveDataModel?.runtimePartyDataModel.actors.Count)
                {
                    for (var j = 0; j < _runtimeSaveDataModel.runtimeActorDataModels.Count; j++)
                    {
                        if (_runtimeSaveDataModel.runtimeActorDataModels[j].actorId == _runtimeSaveDataModel.runtimePartyDataModel.actors[i])
                        {
