...
namespace RPGMaker.Codebase.Runtime.Common.Component.Hud.Actor
{
    public class ActorChangeExp
    {
        ...
        private void ChangeEXPProcess(int exp, string actorId) {
            ...
            //事前のレベルを取得
            var beforeLevel = DataManager.Self().GetGameParty().Actors[index].Level;
            //経験値変動
            DataManager.Self().GetGameParty().Actors[index].LevelupMessage = new List<string>();
            DataManager.Self().GetGameParty().Actors[index].GainExp(exp, false);
            //レベルが上がったかの確認
            isLevelUp = beforeLevel < DataManager.Self().GetGameParty().Actors[index].Level;

            _messages = new List<string>();
            if (isLevelUp)
            {
               var text = DataManager.Self().GetGameParty().Actors[index].LevelUpText();
                for (int i = 0; i < DataManager.Self().GetGameParty().Actors[index].LevelupMessage.Count; i++)
                {
                    if ((i + 1) % 4 == 0)
                    {
                        _messages.Add(text);
                        text = DataManager.Self().GetGameParty().Actors[index].LevelupMessage[i];
                    }
                    else
                    {
                        text += "\\!\n" + DataManager.Self().GetGameParty().Actors[index].LevelupMessage[i];
                    }
                }
                _messages.Add(text);
            }
