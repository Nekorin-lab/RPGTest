...
namespace RPGMaker.Codebase.Runtime.Common.Component.Hud.Character
{
    ...
    public class MoveSetMovePoint : MonoBehaviour
    {
        ...
        /// </summary>
        private const float BaseMoveSpeed = 3.75f;
        ...
        public void Init() {
            ...
            _prefab.transform.parent = gameObject.transform;
        ...
        private List<CharacterOnMap> GetCharacterOnMaps() {
            ...
            if (_moveKind == 0 && _targetCharacer.IsPlayer && MapManager.PartyOnMap != null)
            {
            ...
            switch (_moveKind)
            {
                ...
                case 1: //random
                    ...
                    if (!_tilesOnThePosition.CanEnterThisTiles(directionEnum, vehicleId))
                    {
                        // 進入できない場合は向きだけ変える
                        // ランダム移動の場合は、向きを変更しない
                        //_targetObj.GetComponent<CharacterOnMap>().ChangeCharacterDirection(directionEnum);
        ...
        private bool CanMove(CharacterMoveDirectionEnum directionEnum) {
            ...
            var x_next = _parent.x_now;
            var y_next = _parent.y_now;
