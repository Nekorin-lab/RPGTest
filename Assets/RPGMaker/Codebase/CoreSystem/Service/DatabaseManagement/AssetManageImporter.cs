    ...
    public static class AssetManageImporter
    {
        ...
        private static bool DisplayDialog(string process, string addText = "") {
            ...
            switch (process)
            {
                ...
                case PROCESS_CANCEL_TEXT:
                    //EditorUtility.DisplayDialog(CoreSystemLocalize.LocalizeText(PROCESS_TEXT),
                    //    CoreSystemLocalize.LocalizeText(PROCESS_CANCEL_TEXT),
                    //    CoreSystemLocalize.LocalizeText(CLOSE_TEXT));
                    break;
        ...
        public static List<T> ImportZip_Effect<T>(
            ...
            using (var archive = ZipFile.OpenRead(path))
            {
                ...
                {
                    // ファイルが見つからない
                    //Debug.LogError("FILE_NOT_FOUND: *.prefab, *.efkefc");
                    return null;
                }
                else if (DataFoundInZip(archive, ".prefab") && DataFoundInZip(archive, ".efkefc"))
                {
                    // ダイアログ表示
                    //Debug.LogError("FILE_CONFLICT: *.prefab & *.efkefc");
                    return null;
                }
                else
                {
                ...
                for (var i = 0; i < fileType.Count; i++)
                {
                    ...
                    {
                        // ファイルが見つからない
                        //Debug.LogError($"FILE_NOT_FOUND_TEXT: {fileType[i]}");//素材はあったりなかったりするので、ここではログ出力しない。
                ...
                {
                    // エラーダイアログ表示
                    //Debug.LogError("PROCESS_FAIL: insufficient files");
                    return null;
                }
                ...
                for (var i = 0; i < fileType.Count; i++)
                {
                    ...
                    foreach (var entry in targetFile)
                    {
                        ...
                        if (fileType[i] == "json")
                        {
                                ...
                                Encoding.GetEncoding("shift_jis")))
                            {
                                ...
                                catch
                                {
                                    // キャスト失敗
                                    // ダイアログ表示
                                    //Debug.LogError($"PROCESS_FAIL: Failed to cast: {entry.FullName}");
                                }
                            }
                        ...
                        if (File.Exists(savePath[i] + entry.FullName))
                        {
                            //Debug.Log($"PROCESS_OVERWRITE: {entry.FullName}");
