...
namespace RPGMaker.Codebase.CoreSystem.Knowledge.DataModel.Map
{
    ...
    public class MapDataModel : WithSerialNumberDataModel
    {
        ...
        public class Layer
        {
            ...
            public        List<TileDataModel> tilesOnPalette;

            public LayerType type;

            public Layer(LayerType type, Tilemap tilemap, List<TileDataModel> tilesOnPalette) {
                this.type = type;
            ...
            public Layer(
                ...
                Vector2 cellSize,
                Tilemap tilemap,
                List<TileDataModel> tilesOnPalette
            ) {
                this.type = type;
                this.grid = grid;
                this.tilemap = tilemap;
                tilemap.transform.parent = grid.gameObject.transform;
        ...
        public class MapPrefabManager
        {
            ...
            public List<Layer> LoadLayers()
            {
                ...
                var tileEntities = new TileRepository().GetTileEntities();
                _mapLayers = null;
                for (int index = 0; index < mapDataModel.layerJsons.Count; index++)
                {
                    ...
                    List<TileDataModel> tilesOnPalette = new List<TileDataModel>();
                    for (int i = 0; i < mapDataModel.layerJsons[index].tileIdsOnPalette.Count; i++)
                    {
                        TileDataModel data = null;
