...
namespace RPGMaker.Codebase.CoreSystem.Helper
{
    public static class ScriptableObjectOperator
    {
        ...
        public static void CreateSO() {
            ...
            /*
            dataPath = Directory.GetFiles(JSON_PATH_MAP, "*.json", SearchOption.TopDirectoryOnly);
            for (var i = 0; i < dataPath.Length; i++)
            {
                dataPath[i] = dataPath[i].Replace("\\", "/");

                jsonString = UnityEditorWrapper.AssetDatabaseWrapper.LoadJsonString(dataPath[i]);
                var mapData = new MapRepository().ConvertJsonToEntity(JsonHelper.FromJson<MapJson>(jsonString));
                var mapDataSo = ScriptableObject.CreateInstance<MapSO>();
                mapDataSo.dataModel = mapData;
                if (mapData.id != "")
                {
                    CreateAsset(mapDataSo, SO_PATH_MAP + "/" + mapData.id + ".asset");
#if UNITY_EDITOR
                    AddressableManager.Path.SetAddressToAsset(SO_PATH_MAP + "/" + mapData.id + ".asset");
#endif
                }

                MapBaseDataModel work = new MapBaseDataModel(mapData.id, mapData.name);
