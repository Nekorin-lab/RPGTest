...
namespace RPGMaker.Codebase.Editor.Inspector.Map.View
{
    public class TileInspector : AbstractInspectorElement
    {
        ...
        public TileInspector(TileDataModel tileDataModel, Action onClickRegisterBtn, TYPE inspectorType = TYPE.NORMAL) {
            ...
            _isFindTile = false;
            var tiles = _mapManagementService.LoadTiles();
        ...
        private List<TileDataModel> GetLargePartsTileDataModels(TileDataModel tileDataModel) {
            ...
            var tiles = _mapManagementService.LoadTiles();
            return tiles.Where(t => t.type == TileDataModel.Type.LargeParts &&
                                            t.largePartsDataModel.parentId == parentId).ToList();
        }

        private void Delete(TileDataModel tileDataModel) {
            var path = "Assets/RPGMaker/Storage/Map/TileAssets/";

            // ファイルとフォルダを削除
            if (File.Exists(path + tileDataModel.id + ".asset"))
                File.Delete(path + tileDataModel.id + ".asset");
            if (File.Exists(path + tileDataModel.id + ".asset.meta"))
                File.Delete(path + tileDataModel.id + ".asset.meta");
            if (File.Exists(path + tileDataModel.id + ".meta"))
                File.Delete(path + tileDataModel.id + ".meta");
            if (Directory.Exists(path + tileDataModel.id))
                Directory.Delete(path + tileDataModel.id, true);
            UnityEditorWrapper.AssetDatabaseWrapper.Refresh2();
            _mapManagementService.RemoveTile(tileDataModel);
        }
    }
}