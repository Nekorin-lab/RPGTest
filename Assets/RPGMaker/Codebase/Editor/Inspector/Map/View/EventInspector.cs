    ...
    public class EventInspector : AbstractInspectorElement
    {
        ...
        override protected void InitializeContents() {
            ...
            // アウトラインエディター関連。
            {
                ...
                void SetChapterPopupField() {
                    ...
                    {
                        //int.TryParse(_chapterPopupField.value, out var chapterCode);
                ...
                void SetSectionPopupField() {
                    ...
                    {
                        //int.TryParse(_sectionPopupField.value, out var sectionCode);
        ...
        private void SetCallbackToUi() {
            ...
            // SDキャラクター画像ボタン。
            {
                ...
                if (_targetEventMapPage.image.sdName != "")
                {
        ...
        private void OnClickIcon(int index) {
            var characterSelectModalWindow = new ImageSelectModalWindow(PathManager.IMAGE_FACE, true);
            characterSelectModalWindow.ShowWindow(EditorLocalize.LocalizeWindowTitle("Select IconImage"), data =>
            {
                var imageName = (string) data;
                _eventMapDataModel.pages[index].image.faceName = imageName;
                var imageFace = ImageManager.LoadFace(imageName);
                BackgroundImageHelper.SetBackground(
                    _eventCharacterImage,
                    new Vector2(
                        _eventCharacterImageContainer.style.width.value.value,
                        _eventCharacterImageContainer.style.height.value.value),
                    imageFace,
                    LengthUnit.Pixel);
                _Save();
                MapEditor.MapEditor.SaveEventMap(_eventMapDataModel);
            }, _eventMapDataModel.pages[index].image.faceName);
        }
        ...
        private void UpdateData() {
            _ = Editor.Hierarchy.Hierarchy.Refresh(Editor.Hierarchy.Enum.Region.Map, _mapDataModel.id);
            //Editor.Hierarchy.Hierarchy.Refresh(Editor.Hierarchy.Enum.Region.Outline);
