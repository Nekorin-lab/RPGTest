    ...
    public class AssetManageInspectorElement : AbstractInspectorElement
    {
        ...
        override protected void InitializeContents() {
            ...
            else
            {
                ...
                _sceneView.Init();
                //_sceneView.GetManagePreview().SetImageSetting(frameList, speedList);
                _sceneView.Render();
            }
            ...
            // トグル
            for (var i = 0; i < RootContainer.Query<Toggle>("weapon_type_toggle").ToList().Count; i++)
            {
                var toggle = RootContainer.Query<Toggle>("weapon_type_toggle").AtIndex(i);
                toggle.value = _assetManageDataModel.weaponTypeId == i;
                toggle.SetEnabled(_assetManageDataModel.weaponTypeId != i);
                var num = i;
                toggle.RegisterValueChangedCallback(o =>
                {
                    if (toggle.value)
                    {
                        toggle.SetEnabled(false);
                        _assetManageDataModel.weaponTypeId = num;

                        for (var j = 0; j < RootContainer.Query<Toggle>("weapon_type_toggle").ToList().Count; j++)
                        {
                            var toggle2 = RootContainer.Query<Toggle>("weapon_type_toggle").AtIndex(j);
                            if (num != j)
                            {
        ...
        private void UpdateData() {
            _ = Editor.Hierarchy.Hierarchy.Refresh(Region.AssetManage);
        }
    }
}