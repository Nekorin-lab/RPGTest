...
namespace RPGMaker.Codebase.Editor.MapEditor.Component.CommandEditor.FlowControl
{
    ...
    public class ConditionalBranch : AbstractCommandEditor
    {
        ...
        public override void Invoke() {
            ...
            // for (var i = 0; i < actorSubToggleList.Count; i++)
            // {
            //     var index = i;
            //     actorSubToggleList[index].RegisterValueChangedCallback(evt =>
            //     {
            //         if (actorSubToggleList[index].value)
            //         {
            //             _targetCommand.parameters[21] = index.ToString();
            ...
            //                 if (index != j)
            //                 {
            //                     actorSubToggleList[j].value = false;
            ...
            selectID = 0;
            selectID = enemyIDArray.IndexOf(_targetCommand.parameters[29]);
            if (selectID == -1)
                selectID = 0;
            var enemyPopupField = new PopupFieldBase<string>(enemyNameArray, selectID);
            enemyList.Clear();
            enemyList.Add(enemyPopupField);
            enemyPopupField.RegisterValueChangedCallback(evt =>
            {
                _targetCommand.parameters[29] = enemyIDArray[enemyNameArray.IndexOf(enemyPopupField.value)];
        ...
        private void MainToggleInit() {
            ...
            if (_targetCommand.parameters[15] == "1")
            {
                ...
                //int time = 0;
                //if (!int.TryParse(EventScriptableObject.dataModels[EventIndex].eventCommands[EventCommandIndex]
                //    .parameters[16], out time))
                //{
                //    time = 0;
                //}
                //parameters[16] = time.ToString();
                        ...
                        switch (kind)
                        {
                            ...
                            case FlowKind.ENEMY:
                                ...
                                _targetCommand.parameters[29] =
                                    _enemyDataModels.Count > 0 ? _enemyDataModels[selectID].id : "";
                                selectID = 0;
                                selectID = stateIDArray.IndexOf(_targetCommand.parameters[31]);
                                if (selectID == -1)
                                    selectID = 0;
        ...
        private void ToggleActiveSettings(int index) {
            ...
            for (var j = 0; j < mainToggleList.Count; j++)
            {
                if (index != j && _targetCommand.parameters[1] != "1")
                {
                    mainToggleList[j].value = false;
                    mainToggleList[j].SetEnabled(true);
                }
