...
namespace RPGMaker.Codebase.Editor.MapEditor.Component.Inventory
{
    ...
    public class TileInventory : VisualElement
    {
        ...
        // 大型パーツタイル用
        public List<TileDataModel> TleEntities { get; private set; }
         ...
         * データおよび表示を更新
         */
        public void Refresh([CanBeNull] List<TileDataModel> tiles = null) {
            if (tiles != null) TleEntities = tiles;
         ...
         */
        private List<TileDataModel> GetTilesOfCurrentType() {
            return TleEntities.FindAll(tile => tile.type.Equals(CurrentTileType)).ToList();
        }
    }
}