    ...
    public class LayerInventory : VisualElement
    {
        ...
        private static readonly Dictionary<MapDataModel.Layer.LayerType, string> LayerTypeWoadList =
            new Dictionary<MapDataModel.Layer.LayerType, string>
            {
                ...
                {MapDataModel.Layer.LayerType.ForRoute, "WORD_0784"} //いらないかも
            };
        ...
        public void SetTilesInCurrentLayer(List<TileDataModel> tiles) {
            CurrentLayer.tilesOnPalette = tiles;
        }

        /**
         * 現在表示中のレイヤーにタイルを追加
         */
        public void AddTileToCurrentLayer(TileDataModel tileDataModel, List<TileDataModel> tleEntities) {
            if (CurrentLayer.tilesOnPalette == null)
                return;

            if (CurrentLayer.tilesOnPalette.Contains(tileDataModel)) return;
            ...
            {
                CurrentLayer.tilesOnPalette.Add(tileDataModel);
            }
        ...
        public void RemoveTileFromCurrentLayer(TileDataModel tileDataModel) {
            if (!CurrentLayer.tilesOnPalette.Contains(tileDataModel))
                return;

            if (tileDataModel.type != TileDataModel.Type.LargeParts)
            {
                CurrentLayer.tilesOnPalette.Remove(tileDataModel);
            }
            else
            {
