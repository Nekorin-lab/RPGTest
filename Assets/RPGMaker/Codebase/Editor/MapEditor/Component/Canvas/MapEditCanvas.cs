...
namespace RPGMaker.Codebase.Editor.MapEditor.Component.Canvas
{
    ...
    public class MapEditCanvas : MapCanvas
    {
            ...
            void LayersProcessForEditor(ref TileDataModel shadowToDraw)
            {
                ...
                foreach (var layer in layers)
                {
                    if (layer.type == MapDataModel.Layer.LayerType.Shadow)
                    {
                        ...
                        layer.tilesOnPalette.Add(tileDataModel);
                        shadowToDraw = layer.tilesOnPalette[0];
                        break;
                    }
                }
        ...
        private void Fill(Vector3Int cellPos, TileDataModel fillTarget) {
            // 描画対象タイルとターゲットのタイルが同一
            if (_currentTileToDraw == fillTarget) return;

            var destTile = _currentTargetMapLayer.GetTileDataModelByPosition(new Vector2(cellPos.x, cellPos.y));
            
            // 描画対象タイルと同一タイル
            if (_currentTileToDraw == destTile) return;
            // ターゲットのタイルと同一のタイルでない
            if (destTile != fillTarget || destTile?.id != fillTarget?.id) return;

            if (cellPos.x < MapDataModel.width && cellPos.x >= 0 && 
                cellPos.y > -MapDataModel.height && cellPos.y <= 0)
            {
                bool isDraw = SetTile(cellPos, _currentTileToDraw);
                if (isDraw)
                {
                    Fill(cellPos + new Vector3Int(0, 1), fillTarget);
                    Fill(cellPos + new Vector3Int(0, -1), fillTarget);
                    Fill(cellPos + new Vector3Int(1, 0), fillTarget);
                    Fill(cellPos + new Vector3Int(-1, 0), fillTarget);
                }
            }
        }
        ...
        public void ChangeMapSizeForEditor(int width, int height) {
            ...
            MapEditor.SaveMap(MapDataModel, MapRepository.SaveType.SAVE_PREFAB_FORCE);
        ...
        public void DeleteEvent(int width, int height) {
            ...
            _ = Editor.Hierarchy.Hierarchy.Refresh(Region.Map, MapDataModel.id);
        }
