        ...
        public override VisualElement Invoke(string indent, EventDataModel.EventCommand eventCommand) {
            ...
            switch ((FlowKind) kind)
            {
                ...
                case FlowKind.ENEMY:
                    ...
                    var enemy = _GetEnemyList();
                    var enemyData = enemy.FirstOrDefault(c => c.id == eventCommand.parameters[1]);
                    var enemyName = enemyData.name;
                    if (enemyName == "") enemyName = EditorLocalize.LocalizeText("WORD_1518");
                    ret += "#" + (enemy.IndexOf(enemyData) + 1).ToString("0000") + enemyName + " ";
                    if (int.Parse(eventCommand.parameters[2]) == 0)
                    {
                    ...
                    else
                    {
                        //ret += "is affected by ";
                        var stateDataModels = DatabaseManagementService.LoadStateEdit();
                        ret += stateDataModels.FirstOrDefault(c => c.id == eventCommand.parameters[1])?.name +
                               EditorLocalize.LocalizeText("WORD_1147");
                    }
