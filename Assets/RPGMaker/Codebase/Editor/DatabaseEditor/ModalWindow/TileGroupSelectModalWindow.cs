...
namespace RPGMaker.Codebase.Editor.DatabaseEditor.ModalWindow
{
    public class TileGroupSelectModalWindow : BaseModalWindow
    {
        ...
        private static List<TileDataModel>      _tileData;
        ...
        public void ShowSaveWindow(string modalTitle, CallBackWidow callBack, List<TileDataModel> tileData) {
            _isSave = true;
            _tileData = tileData;
        ...
        private Button CreateAddButton(VisualElement visualElement) {
            ...
            {
                // データを作成して追加、セーブする
                _tileGroupEntities.Add(new TileGroupDataModel(
                    Guid.NewGuid().ToString(), EditorLocalize.LocalizeText("WORD_1596"), new List<TileDataModel>()));
                _mapManagementService.SaveTileGroup(_tileGroupEntities[_tileGroupEntities.Count - 1]);
                visualElement.Clear();
