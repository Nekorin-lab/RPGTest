...
namespace RPGMaker.Codebase.Editor.DatabaseEditor.ModalWindow
{
    ...
    public class DlcImporterWindow : AssetPostprocessor
    {
        ...
        static string _thereAreaSameNameAssetsText = "WORD_2906";
        ...
        static FieldInfo _tileImageEntitiesFi;
        ...
        static string[] GetDirectoriesRecur(string path, string filename) {
            ...
            {
                list.AddRange(GetDirectoriesRecur($"{directory}/", filename));
            }
            return list.ToArray();
        }
        ...
        static async Task ImportProcessAsync() {
            try
            {
                ...
                _tileImageEntitiesFi = typeof(RPGMaker.Codebase.Editor.MapEditor.MapEditor).GetField("_tileImageEntities", BindingFlags.NonPublic | BindingFlags.Static);
                ...
                _langIndex = 1;
                var getKeyLanguageMi = typeof(EditorLocalize).GetMethod("GetKeyLanguage", BindingFlags.NonPublic | BindingFlags.Static);
                if (getKeyLanguageMi != null)
                {
                    var systemLanguage = (SystemLanguage) getKeyLanguageMi.Invoke(null, null);
                ...
                var dirs = GetDirectoriesRecur(_dlcBasePath, _importedInfoFilename);
                foreach (var dir in dirs)
                {
                    _currentDlcPath = dir;
                    var filename = $"{_currentDlcPath}{_importedFlagFilename}";
                    string status;
                    if (File.Exists(filename) && ((status = File.ReadAllText(filename)) == "done" || status == "skipped"))
                    {
                        continue;
                    }
                    _dlcName = dir.Substring(_dlcBasePath.Length);
                    ...
                    try
                    {
                        ...
                        if (mapTileRegDicDic.Count > 0)
                        {
                            ...
                            foreach (var dicEntry in mapTileRegDicDic)
                            {
                                ...
                                {
                                    //マップタイルを登録する。
                                    var tileImageDataModels = (List<TileImageDataModel>) _tileImageEntitiesFi.GetValue(null);
                                    ...
                                    if (list.Length >= 2 && _tileTypeNameTypeDic.ContainsKey(list[1]))
                                    {
                                        ...
                                        foreach (var dstFilename in entity.Value)
                                        {
                                            ...
                                            List<TileDataModel> lastTileEntities = null;
                                            if (type == TileDataModel.Type.NormalTile)
                                            {
                                                lastTileEntities = _mapManagementService.LoadTiles();
                                            }
                                            var fn = GetRemovedPathFilename(dstFilename);
                                            ...
                                            if (newTileGroupDataModel != null)
                                            {
                                                ...
                                                if (tileDataModel.type == TileDataModel.Type.LargeParts)
                                                {
                                                    var tileEntities = tileRepository.GetTileEntities();
                                                ...
                                                {
                                                    foreach (var tdm in _mapManagementService.LoadTiles())
                                                    {
                                                ...
                                                else
                                                {
                                                    ...
                                                    {
                                                        newTileGroupDataModel.tileDataModels.Add(tileDataModel);
                                                        tileAdded = true;
                                                    }
                                                }
                        ...
                        if (reloadTiles)
                        {
                            RPGMaker.Codebase.Editor.MapEditor.MapEditor.ReloadTiles();
                        }
                        if (reloadTileGroups)
                        {
        ...
        () {
            var filename = $"{_currentDlcPath}{_importedInfoFilename}";
        ...
        public class MakeChoiceWindow : AddonBaseModalWindow
        {
            ...
            public override void Init() {
                ...
                var label = new Label($"{_dlcName}\n{string.Format(EditorLocalize.LocalizeText(_thereAreaSameNameAssetsText), _collisionFileCount)}");
